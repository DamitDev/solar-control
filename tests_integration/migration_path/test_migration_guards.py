...
async def _create_manual(http_control, host_id: str, alias: str, priority: str = "staging") -> str:
...
async def test_same_host_422(http_control, clean_state):
    ...
    host = hosts["host-a"]
    instance_id = await _create_manual(http_control, host["id"], f"same-{uuid.uuid4().hex[:6]}")
...
async def test_active_training_blocks_migration(http_control, http_host, clean_state):
    ...
    hosts = await _hosts(http_control)
    src, dst = hosts["host-a"], hosts["host-b"]
    instance_id = await _create_manual(http_control, src["id"], f"train-{uuid.uuid4().hex[:6]}")
    ...
    resp = await http_host.post(
        ...
        json={
            ...
            "steps": [{"name": "train", "image": "imgrepo.damit.hu/supernova/no-such-image:v1"}],
        },
    )
...
async def test_no_target_ephemeral_stop(http_control, stack, clean_state):
    ...
    low_replicas = {r["host_id"]: r["instance_id"] for r in low_ready["status"]["replica_set"]}
